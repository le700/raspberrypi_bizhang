...
- 树莓派上用的是 **OpenCV DNN**，它**不认识 `.pt` 文件**
- 所以需要把 `best.pt` 转换成 OpenCV 能读的格式

**树莓派不需要下载原始的 yolov3-tiny！** 只需要用你自己训练出来的模型！
...
│                        电脑（训练环境）                           │
├─────────────────────────────────────────────────────────────────┤
│  收集图片 → 标注 → 训练 → best.pt → 导出 → best.cfg + best.weights │
└─────────────────────────────────────────────────────────────────┘
                                    ↓
                              传到树莓派
                                    ↓
...
# 导出为 Darknet 格式（推荐，和当前代码完美匹配）
# 会生成：best.cfg 和 best.weights
model.export(format='darknet')
...
├── raspberrypi_bizhang.py    ← 主程序（已在仓库里）
├── best.cfg                  ← 从训练电脑传过来 ⭐
└── best.weights              ← 从训练电脑传过来 ⭐
```
...
- 命令行方式（从电脑传到树莓派）：
```bash
scp best.cfg pi@192.168.x.x:/home/pi/raspberrypi_bizhang/
scp best.weights pi@192.168.x.x:/home/pi/raspberrypi_bizhang/
```
...
改成这样：
```python
YOLO_NET = cv2.dnn.readNetFromDarknet('best.cfg', 'best.weights')
```
...
#### Q1: 树莓派上还需要下载yolov3-tiny吗？

**答**：完全不需要！只需要你自己训练出来的 `best.cfg` 和 `best.weights`！

#### Q2: 树莓派上需要安装PyTorch吗？

**答**：不需要！树莓派只需要OpenCV和numpy，代码用的是OpenCV DNN，不需要PyTorch！

#### Q3: 树莓派上模型加载报错怎么办？

**答**：
1. 检查 `best.cfg` 和 `best.weights` 是否在同一目录
2. 检查文件名是否正确（注意大小写）
3. 检查模型是否正确导出了
4. 尝试用绝对路径加载
